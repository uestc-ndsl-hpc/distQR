...
template <typename T>
inline void PackCompactChunkToPackedBroadcastCache(int k_chunk_begin,
                                                   int kc,
                                                   int block_rows,
                                                   const T* d_block_w,
                                                   const T* d_block_y,
    ...
    AssertCuda(cudaMemcpy2DAsync(dst_w, static_cast<size_t>(chunk_rows) * sizeof(T), src_w,
                                 static_cast<size_t>(block_rows) * sizeof(T),
                                 static_cast<size_t>(chunk_rows) * sizeof(T), kc,
                                 cudaMemcpyDeviceToDevice, stream),
               "cudaMemcpy2DAsync compact chunk W -> packed cache");
    AssertCuda(cudaMemcpy2DAsync(dst_y, static_cast<size_t>(chunk_rows) * sizeof(T), src_y,
                                 static_cast<size_t>(block_rows) * sizeof(T),
                                 static_cast<size_t>(chunk_rows) * sizeof(T), kc,
                                 cudaMemcpyDeviceToDevice, stream),
               "cudaMemcpy2DAsync compact chunk Y -> packed cache");
}

template <typename T>
inline void UnpackPackedCompactChunkToCompactBlockStorage(int k_chunk_begin,
                                                          int kc,
                                                          int block_rows,
                                                          const T* d_wy_packed,
                                                          T* d_block_w,
    ...
    AssertCuda(cudaMemcpy2DAsync(dst_w, static_cast<size_t>(block_rows) * sizeof(T), src_w,
                                 ...
                                 static_cast<size_t>(chunk_rows) * sizeof(T), kc,
                                 cudaMemcpyDeviceToDevice, stream),
               "cudaMemcpy2DAsync packed chunk W -> compact storage");
    AssertCuda(cudaMemcpy2DAsync(dst_y, static_cast<size_t>(block_rows) * sizeof(T), src_y,
                                 static_cast<size_t>(chunk_rows) * sizeof(T),
                                 static_cast<size_t>(chunk_rows) * sizeof(T), kc,
                                 cudaMemcpyDeviceToDevice, stream),
               "cudaMemcpy2DAsync packed chunk Y -> compact storage");
}
...
template <typename T>
inline void TailAccumulateCompactChunkToTile(cublasHandle_t tail_cublas_handle,
                                             const T* d_block_w,
                                             int block_begin,
                                             int block_rows,
                                             int kb,
    ...
    const size_t tail_acc_gemm_idx =
        BeginPhaseInterval(phase_profile, PhaseKind::TailAccGemm, tail_update_stream);
    AssertCublas(CublasGemmTraits<T>::Gemm(tail_cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, kc,
                                           cols_tile, chunk_rows, &one, w_chunk, block_rows, a_sub,
                                           lda_local, &zero, tmp_sub, kb),
                 "Pipeline chunked tail tmp = W_chunk^T * A_sub");
    EndPhaseInterval(phase_profile, tail_acc_gemm_idx, tail_update_stream);
}

template <typename T>
inline void TailApplyCompactChunkToTile(cublasHandle_t tail_cublas_handle,
                                        const T* d_block_y,
                                        int block_begin,
                                        int block_rows,
                                        int kb,
