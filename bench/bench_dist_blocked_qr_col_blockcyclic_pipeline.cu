...
int RunBenchmarkTyped(const MpiCudaEnv& env, const Options& opts, int block_cols) {
    ...
    std::vector<double> phase_comm_w_ms_per_iter;
    std::vector<double> phase_comm_y_ms_per_iter;
    ...
    std::vector<double> comm_w_bytes_per_iter;
    ...
    if (opts.print_phase_timing) {
        ...
        phase_comm_w_ms_per_iter.resize(opts.iters, 0.0);
        phase_comm_y_ms_per_iter.resize(opts.iters, 0.0);
        ...
        comm_w_bytes_per_iter.resize(opts.iters, 0.0);
    ...
    for (int i = 0; i < opts.iters; ++i) {
        ...
        if (opts.print_phase_timing) {
            ...
            phase_comm_w_ms_per_iter[i] = phase_profile.comm_w_ms;
            phase_comm_y_ms_per_iter[i] = phase_profile.comm_y_ms;
            ...
            comm_w_bytes_per_iter[i] = static_cast<double>(comm_profile.bytes_w);
    ...
    std::vector<double> all_comm_w_ms;
    std::vector<double> all_comm_y_ms;
    ...
    std::vector<double> all_comm_w_bytes;
    std::vector<double> all_comm_y_bytes;
    std::vector<double> all_comm_w_useful_bytes;
    ...
    double local_comm_w_ms = 0.0;
    double local_comm_y_ms = 0.0;
    ...
    if (opts.print_phase_timing) {
        for (int i = 0; i < opts.iters; ++i) {
            ...
            local_comm_w_ms += phase_comm_w_ms_per_iter[i];
            local_comm_y_ms += phase_comm_y_ms_per_iter[i];
            ...
            local_comm_w_bytes += comm_w_bytes_per_iter[i];
        ...
        local_comm_w_ms /= static_cast<double>(opts.iters);
        local_comm_y_ms /= static_cast<double>(opts.iters);
        ...
        local_comm_w_bytes /= static_cast<double>(opts.iters);
    ...
    const double local_comm_w_gbps =
        GigabytesPerSecondFromBytesAndMs(local_comm_w_bytes, local_comm_w_ms);
    const double local_comm_y_gbps =
        GigabytesPerSecondFromBytesAndMs(local_comm_y_bytes, local_comm_y_ms);
    ...
    if (opts.print_phase_timing && env.rank == 0) {
        ...
        all_comm_w_ms.resize(env.size, 0.0);
        all_comm_y_ms.resize(env.size, 0.0);
        ...
        all_comm_w_bytes.resize(env.size, 0.0);
        all_comm_y_bytes.resize(env.size, 0.0);
        all_comm_w_useful_bytes.resize(env.size, 0.0);
    ...
    if (opts.print_phase_timing) {
        ...
        MPI_Gather(&local_unpack_ms, 1, MPI_DOUBLE,
                   ...
                   MPI_COMM_WORLD);
        MPI_Gather(&local_comm_w_ms, 1, MPI_DOUBLE,
                   (env.rank == 0) ? all_comm_w_ms.data() : nullptr, 1, MPI_DOUBLE, 0,
                   MPI_COMM_WORLD);
        MPI_Gather(&local_comm_y_ms, 1, MPI_DOUBLE,
                   (env.rank == 0) ? all_comm_y_ms.data() : nullptr, 1, MPI_DOUBLE, 0,
                   MPI_COMM_WORLD);
        ...
        MPI_Gather(&local_comm_bytes, 1, MPI_DOUBLE,
                   ...
                   MPI_COMM_WORLD);
        MPI_Gather(&local_comm_w_bytes, 1, MPI_DOUBLE,
                   (env.rank == 0) ? all_comm_w_bytes.data() : nullptr, 1, MPI_DOUBLE, 0,
                   MPI_COMM_WORLD);
        MPI_Gather(&local_comm_y_bytes, 1, MPI_DOUBLE,
                   (env.rank == 0) ? all_comm_y_bytes.data() : nullptr, 1, MPI_DOUBLE, 0,
                   MPI_COMM_WORLD);
        MPI_Gather(&local_comm_w_useful_bytes, 1, MPI_DOUBLE,
                   (env.rank == 0) ? all_comm_w_useful_bytes.data() : nullptr, 1, MPI_DOUBLE, 0,
                   MPI_COMM_WORLD);
        MPI_Gather(&local_comm_y_useful_bytes, 1, MPI_DOUBLE,
                   (env.rank == 0) ? all_comm_y_useful_bytes.data() : nullptr, 1, MPI_DOUBLE, 0,
                   MPI_COMM_WORLD);
        MPI_Gather(&local_comm_w_gbps, 1, MPI_DOUBLE,
                   (env.rank == 0) ? all_comm_w_gbps.data() : nullptr, 1, MPI_DOUBLE, 0,
                   MPI_COMM_WORLD);
        ...
        if (opts.print_phase_timing) {
            for (int r = 0; r < env.size; ++r) {
                ...
                spdlog::info(
                    ...
                    "inner_update {:.3f} ms, pack {:.3f} ms, unpack {:.3f} ms, "
                    "comm_w {:.3f} ms, comm_y {:.3f} ms, comm {:.3f} ms, tail_wait {:.3f} ms, "
                    "tail_acc_gemm {:.3f} ms, tail_apply_gemm {:.3f} ms",
                    r, all_panel_ms[r], all_wy_ms[r], all_merge_ms[r], all_inner_update_ms[r],
                    all_pack_ms[r], all_unpack_ms[r], all_comm_w_ms[r], all_comm_y_ms[r],
                    all_comm_ms[r], all_tail_wait_ms[r], all_tail_acc_gemm_ms[r],
                    all_tail_apply_gemm_ms[r]);
            }
        }
        if (total_bad > 0) {
